...
const courseParamMap: Record<string, string> = {
  "fundamentos-programacion-i": "programacion1",
  "fundamentos-programacion-ii": "programacion2",
  roblox: "roblox",
  unity: "unity",
  "unity-ii": "unity2",
  "desarrollo-web": "web",
  "inteligencia-artificial-i": "ia",
  "inteligencia-artificial-ii": "ia2",
};
...
export default function SolicitarPlazaPage() {
  ...
  useEffect(() => {
    const rawCourse = searchParams.get("curso") ?? searchParams.get("course");
    if (!rawCourse) return;

    const normalized = rawCourse.toLowerCase();
    const mapped = courseParamMap[normalized] ?? normalized;
    const selectedCourse = cursosOptions.find((option) => option.value === mapped);

    if (!selectedCourse || !selectedCourse.value) return;

    setFormData((prev) => {
      if (prev.curso) return prev;
      return {
        ...prev,
        curso: selectedCourse.value,
        mensaje:
          prev.mensaje ||
          `Me interesa especialmente el curso: ${selectedCourse.label}.`,
      };
    });
  }, [searchParams]);
  ...
  return (
    <div className="min-h-screen bg-white">
        ...
        <div className="relative mx-auto max-w-7xl px-6 py-16 md:py-24">
          ...
          >
            ...
            <div className="mt-10 flex flex-col sm:flex-row items-center justify-center gap-4">
              <Link href="#form" className="btn-primary btn-cta w-full sm:w-auto rounded-full">
                Rellenar solicitud
              </Link>
              <Link href="/cursos" className="btn-secondary btn-cta w-full sm:w-auto rounded-full">
                Ver cursos
              </Link>
            </div>
          </motion.div>
      ...
      {/* CONTENT */}
      <section id="form" className="py-16 md:py-20 px-6 scroll-mt-24">
            ...
            {ok ? (
              ...
              >
                ...
                <div className="mt-8 flex flex-col sm:flex-row gap-3 justify-center">
                  <Link
                    href="/cursos"
                    className="btn-secondary btn-cta rounded-full"
                  >
                    Ver cursos
                  </Link>
                  <Button
                    className="btn-primary btn-cta rounded-full"
                    onClick={() => setOk(false)}
                  >
            ...
            >
              ...
              <Card className="rounded-3xl border border-black/10 bg-white p-7 shadow-sm">
                ...
                <div className="mt-5 space-y-3">
                  <a href="tel:+34624547406" className="btn-primary btn-cta w-full rounded-full">
